...
class JobCategoryController extends Controller
{
    ...
    {
        if (App::getLocale() == 'ar') {
            $sections = JobCategoryMdl::orderBy('name_ar', 'ASC')->paginate(pageCount);
        } else {
            $sections = JobCategoryMdl::orderBy('name_en', 'ASC')->paginate(pageCount);
        }

        $sectionsCount = JobCategoryMdl::count();

        $list_title = __('general.All');

        return view('jobCategory.index', compact('sections', 'sectionsCount', 'list_title'));
    }
    /**
     * ============================
     * ============================
    ...
    public function store(Request $req)
    {
        $valid = Validator::make($req->all(), [
            'name_ar' => 'required|unique:jobs_categories,name_ar|string|min:3',
            'name_en' => 'required|unique:jobs_categories,name_en|string|min:3',
        ], [
    ...
    {
        $section  = JobCategoryMdl::find($category);

        return view('jobCategory.edit', compact('section'));
    }
    /**
     * ============================
     * ============================
    ...
    {
        $valid = Validator::make($req->all(), [
            'section_id' => 'required|integer|exists:jobs_categories,id',
            'name_ar' => 'required|unique:jobs_categories,name_ar,' . $req->section_id . '|string|min:3',
            'name_en' => 'required|unique:jobs_categories,name_en,' . $req->section_id . '|string|min:3',
        ], [
        ...
        $category = jobCategoryMdl::findOrFail($req->section_id);
    ...
    {
        $valid = Validator::make($req->all(), [
            'sectionID' => 'required|integer|exists:jobs_categories,id',
        ]);
        ...
        $category = JobCategoryMdl::findOrFail($req->sectionID);
