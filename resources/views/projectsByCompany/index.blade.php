          ...
          <div class="nk-block-between">
            ...
            <div class="nk-block-head-content">
              ...
              <div class="nk-block-des text-soft">
                <p>{{ __('general.You have total') }} {{ number_format($projectsCount,0) }}
                  {{ __('project.Project') }}.
                </p>
              </div>
            </div>
              ...
              <div class="card-inner p-0">
                <table class="nk-tb-list nk-tb-ulist">
                  ...
                  <tbody>
                    ...
                    @foreach ($currentProjects as $item)
                    <tr class="nk-tb-item">
                      <td class="nk-tb-col">
                        ...
                        <div class="user-card">
                          <div class="user-avatar sq bg-purple xs">
                            ...
                            <img src="{{ url('storage/app/public/imgs/projects/'.$item->master_photo) }}"
                              alt="{{ $item->name_en }}" />
                            @else
                        ...
                        <ul class="nk-tb-actions gx-1">
                          ...
                          <li class="nk-tb-action-hidden">
                            <a href="#" class="btn btn-trigger btn-icon text-danger" title="{{ __('general.Delete') }}"
                              data-companyid="{{ $company->id }}" data-projectid="{{ $item->id }}" data-toggle="modal"
                            ...
                            <div class="drodown">
                              ...
                              <div class="dropdown-menu dropdown-menu-right">
                                <ul class="link-list-opt no-bdr">
                                  ...
                                  <li>
                                    <a href="#" class="text-danger" data-companyid="{{ $company->id }}"
                                      data-projectid="{{ $item->id }}" data-toggle="modal" data-target="#deleteMdl">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('company.project.store') }}" method="POST">
          @csrf
          <input hidden id="company_id" name="company" value="{{ $company->id }}" />

          <p class="text-center">
          <div class="row gy-4">
            <div class="col">
              <div class="form-group">
                <label class="form-label" for="project_id">{{ __('general.Projects') }}</label>
                <div class="form-control-wrap">
                  <select class="selectpicker form-control" name="project" data-live-search="true" autofocus>
                    <option disabled selected>{{ __('general.Choose') }}</option>
                    @foreach ($projects as $project)
                    @if (App::getLocale()=='ar')
                    <option value="{{ $project->id }}" data-tokens="{{ $project->name_ar }}">{{ $project->name_ar }}
                    </option>
                    @else
                    <option value="{{ $project->id }}" data-tokens="{{ $project->name_en }}">{{ $project->name_en }}
                    </option>
                    @endif
                    @endforeach
...
<div class="modal fade zoon" tabindex="-1" id="deleteMdl">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('company.project.destroy') }}" method="POST">
          @csrf
